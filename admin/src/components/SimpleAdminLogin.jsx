...
function SimpleAdminLogin() {
  ...
  // Admin password
  const ADMIN_PASSWORD = 'newTIMEAMANE31@'
  ...
  const handleLogin = async (e) => {
    ...
    if (!ADMIN_EMAILS.includes(email.toLowerCase())) {
      SecurityLogger.logSecurityEvent('Unauthorized admin login attempt', {
        email: email.toLowerCase(),
        timestamp: Date.now()
      })
      showMessage('This email is not authorized for admin access', 'error')
      return
    }

    // Check password
    if (password !== ADMIN_PASSWORD) {
      SecurityLogger.logSecurityEvent('Incorrect admin password attempt', {
        email: email.toLowerCase(),
        timestamp: Date.now()
      })
