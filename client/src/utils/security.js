// Security utility library for comprehensive website protection
  ...
  monitorFailedLogins() {
    ...
    document.addEventListener('login-attempt', (event) => {
      if (!event.detail.success) {
        failedAttempts++;
        if (failedAttempts >= 5) {
          SecurityLogger.logSecurityEvent('Multiple failed login attempts', {
  ...
  monitorRapidRequests() {
    ...
    window.fetch = function(...args) {
      ...
      if (requestCount > 100) { // More than 100 requests per minute
        SecurityLogger.logSecurityEvent('Suspicious rapid requests', {
          requestCount,
          timestamp: Date.now()
        });
      }
  ...
  monitorXSSAttempts() {
    ...
    console.error = function(...args) {
      ...
      if (message.includes('XSS') || message.includes('Cross-site scripting')) {
        SecurityLogger.logSecurityEvent('Potential XSS attempt', {
          message,
          timestamp: Date.now()
        });
      }
